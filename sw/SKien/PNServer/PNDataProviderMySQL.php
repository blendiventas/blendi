...
class PNDataProviderMySQL implements PNDataProvider
{
     ...
     */
    public function __construct(string $strDBHost, string $strDBUser, string $strDBPwd, string $strDBName, ?string $strTableName = null, ?LoggerInterface $logger = null, $id_terminal = null)
    {
    ...
    public function init(bool $bAutoRemove = true) : bool
    {
        ...
        if ($this->db) {
            ...
            if ($bAutoRemove) {
                ...
                $strWhere .= " WHERE id_terminal = " . $this->id_terminal;
            } else {
                ...
                $bSucceeded = true;
                $strWhere .= " AND id_terminal = " . $this->id_terminal;
            }
            if ($bSucceeded) {
                $strSQL  = "SELECT ";
                $strSQL .= self::COL_ID;
    ...
    public function count() : int
    {
        $iCount = 0;
        if ($this->db) {
            $dbres = $this->db->query("SELECT count(*) AS iCount FROM " . $this->strTableName . " WHERE id_terminal = " . $this->id_terminal);
            if (is_object($dbres)) {
    ...
    private function createTable() : bool
    {
        ...
        if (is_object($this->db)) {
            ...
            $strSQL .= ",id_terminal INT(11) NOT NULL DEFAULT '0'";
