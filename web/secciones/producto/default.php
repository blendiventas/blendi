<?php
$observaciones = (empty($observaciones_producto[0]))? '' : $observaciones_producto[0];
$keyProducto = 0;
$pvp_mostrar = $pvp_producto[0];
$pvp_antes_oferta = $pvp_mostrar;
$esOferta = false;
if ($oferta_desde_producto[0] >= date("Y-m-d") && $oferta_hasta_producto[0] <= date("Y-m-d") && $pvp_producto[0] > $pvp_oferta_producto[0]) {
    $esOferta = true;
    $pvp_mostrar = $pvp_oferta_producto[0];

    $porcentajeOferta = ($pvp_mostrar / $pvp_producto[0] - 1) * 100;
}
?>
<section class="py-20">
    <div class="container mx-auto px-4">
        <div class="flex flex-wrap -mx-4 mb-4 items-center justify-between">
            <div class="w-full lg:w-auto px-4 xl:mb-0">
                <h2 class="text-lg font-bold font-heading">
                    <span>Volver a </span>
                    <a class="relative text-blue-300 underline" href="<?php echo $host_web_tienda . $categoria; ?>"><?php echo $descripcion_categoria; ?></a>
                </h2>
            </div>
        </div>
        <div class="flex flex-wrap -mx-4 mb-24">
            <div class="w-full md:w-1/2 px-4 mb-8 md:mb-0">
                <div class="relative mb-10" style="height: 564px;">
                    <a class="absolute top-1/2 left-0 ml-8 transform translate-1/2" href="#">
                        <svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9 16.0185C9.268 16.2905 9.268 16.7275 9 16.9975C8.732 17.2675 8.299 17.2685 8.031 16.9975L0.201 9.0895C-0.067 8.8195 -0.067 8.3825 0.201 8.1105L8.031 0.2025C8.299 -0.0675 8.732 -0.0675 9 0.2025C9.268 0.4735 9.268 0.9115 9 1.1815L1.859 8.6005L9 16.0185Z" fill="#1F40FF"></path>
                        </svg>
                    </a>
                    <img class="object-contain w-full h-full" src="<?php echo $imagen_producto; ?>" alt="<?php echo $descripcion_producto; ?>">
                    <a class="absolute top-1/2 right-0 mr-8 transform translate-1/2" href="#">
                        <svg width="10" height="18" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M0.19922 1.1817C-0.0687795 0.909696 -0.0687794 0.472695 0.19922 0.202695C0.46722 -0.0673054 0.90022 -0.0683048 1.16822 0.202695L8.99822 8.11069C9.26622 8.3807 9.26622 8.81769 8.99822 9.08969L1.16822 16.9977C0.900219 17.2677 0.467218 17.2677 0.199219 16.9977C-0.0687809 16.7267 -0.0687808 16.2887 0.199219 16.0187L7.34022 8.5997L0.19922 1.1817Z" fill="#1F40FF"></path>
                        </svg>
                    </a>
                </div>
                <div class="flex flex-wrap -mx-2">
                    <div class="w-1/2 sm:w-1/4 p-2">
                        <a class="block border border-blue-300" href="#">
                            <img class="w-full h-32 object-contain" src="<?php echo $imagen_producto; ?>" alt="<?php echo $descripcion_producto; ?>">
                        </a>
                    </div>
                    <?php
                    foreach ($images_producto as $key_images_producto => $value_images_producto) {
                        ?>
                        <div class="w-1/2 sm:w-1/4 p-2">
                            <a class="block border border-transparent hover:border-gray-400" href="#">
                                <img class="w-full h-32 object-contain" src="<?php echo $value_images_producto; ?>" alt="<?php echo $images_alt_producto[$key_images_producto]; ?>">
                            </a>
                        </div>
                        <?php
                    }
                    ?>
                </div>
            </div>
            <div class="w-full md:w-1/2 px-4">
                <div class="lg:pl-20">
                    <div class="mb-10 pb-10 border-b">
                        <!--<span class="text-gray-500">Brille</span>-->
                        <h2 class="mt-2 mb-6 max-w-xl text-5xl md:text-6xl font-bold font-heading"><?php echo $descripcion_producto; ?></h2>
                        <!--<div class="mb-8">
                            <button>
                                <img src="yofte-assets/elements/star-gold.svg" alt="">
                            </button>
                            <button>
                                <img src="yofte-assets/elements/star-gold.svg" alt="">
                            </button>
                            <button>
                                <img src="yofte-assets/elements/star-gold.svg" alt="">
                            </button>
                            <button>
                                <img src="yofte-assets/elements/star-gold.svg" alt="">
                            </button>
                            <button>
                                <img src="yofte-assets/elements/star-gray.svg" alt="">
                            </button>
                        </div>-->
                        <?php
                        if ($packs_disponibles) {
                            $firstPack = true;
                            foreach ($id_packs_producto as $key_pack => $id_pack_producto) {
                                if (isset($cantidad_packs_producto[$key_pack]) && isset($pvp_producto[$key_pack])) {
                                    $pvp_mostrar = $pvp_producto[$key_pack];
                                    $pvp_antes_oferta = $pvp_mostrar;
                                    $esOferta = false;
                                    if ($oferta_desde_producto[$key_pack] >= date("Y-m-d") && $oferta_hasta_producto[$key_pack] <= date("Y-m-d") && $pvp_producto[0] > $pvp_oferta_producto[$key_pack]) {
                                        $esOferta = true;
                                        $pvp_mostrar = $pvp_oferta_producto[$key_pack];

                                        $porcentajeOferta = ($pvp_mostrar / $pvp_producto[$key_pack] - 1) * 100;
                                    }
                                    ?>
                                    <div class="flex inline-block mb-8 text-lg font-bold font-heading text-blue-300 pvp_mostrar <?php echo ($firstPack)? '' : 'hidden'; ?>" id="pvp_mostrar_pack_<?php echo $id_pack_producto; ?>">
                                        <?php
                                        if ($mostrar_precio) {
                                            ?>
                                            <span><?php echo number_format($pvp_mostrar, 2, ',', ''); ?>€</span>
                                            <?php
                                            if ($esOferta) {
                                                ?>
                                                <span class="font-normal text-base text-gray-400 line-through"><?php echo $pvp_antes_oferta; ?>€</span>
                                                <?php
                                            }
                                        } else {
                                            ?>
                                            <span>Precio a consultar.</span>
                                            <?php
                                        }
                                        ?>

                                        <div class="inline-block ml-3 mb-8 text-lg font-heading">
                                            Referencia: <?php echo $referencia_producto[$key_pack]; ?>
                                        </div>
                                    </div>
                                    <?php
                                    $firstPack = false;
                                }
                            }
                        } else {
                            ?>
                            <div class="flex inline-block mb-8 text-lg font-bold font-heading text-blue-300 pvp_mostrar" id="pvp_mostrar">
                                <?php
                                if ($mostrar_precio) {
                                    ?>
                                    <span><?php echo number_format($pvp_mostrar, 2, ',', ''); ?>€</span>
                                    <?php
                                    if ($esOferta) {
                                        ?>
                                        <span class="font-normal text-base text-gray-400 line-through"><?php echo $pvp_antes_oferta; ?>€</span>
                                        <?php
                                    }
                                } else {
                                    ?>
                                    <span>Precio a consultar.</span>
                                    <?php
                                }
                                ?>
                                <div class="inline-block ml-3 mb-8 text-lg font-heading">
                                    Referencia: <?php echo $referencia_producto[0]; ?>
                                </div>
                            </div>
                            <?php
                        }
                        ?>
                        <p class="max-w-md text-gray-500"><?php echo $observaciones; ?></p>
                    </div>
                    <div class="flex mb-12">
                        <?php
                        if ($packs_disponibles && $mostrar_precio) {
                            ?>
                            <div class="mr-6">
                                <span class="block mb-4 font-bold font-heading text-gray-400 uppercase">Elige el pack</span>
                                <div class="inline-flex items-center px-4 font-semibold font-heading text-gray-500 border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md">
                                    <select class="pl-6 pr-6 py-4 border-0 border-gray-100 focus:border-gray-100 focus:ring-transparent focus:outline-none cursor-pointer" name="id_pack" id="id_pack" onchange="mostrarPvpPack()">
                                        <?php
                                        asort($cantidad_packs_producto);
                                        foreach ($cantidad_packs_producto as $key_pack => $qty_pack) {
                                            if (isset($id_packs_producto[$key_pack])) {
                                                $pvp_mostrar_selector_packs = 'S/N';
                                                if ($pvp_producto[$key_pack]) {
                                                    $pvp_mostrar_selector_packs = number_format($pvp_producto[$key_pack], 2, ',', '') . '€';
                                                }
                                                ?>
                                                <option value="<?php echo $id_packs_producto[$key_pack]; ?>"><?php echo $qty_pack; ?> - <?php echo $pvp_mostrar_selector_packs; ?></option>
                                                <?php
                                            }
                                        }
                                        ?>
                                    </select>
                                </div>
                            </div>
                            <?php
                        }
                        if ($producto_stock[$keyProducto] > 0) {
                            ?>
                            <div class="mr-6">
                                <span class="block mb-4 font-bold font-heading text-gray-400 uppercase">Cantidad</span>
                                <div class="inline-flex items-center px-4 font-semibold font-heading text-gray-500 border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md">
                                    <button class="py-2 hover:text-gray-700" onclick="document.getElementById('cantidad_producto').value = parseInt(document.getElementById('cantidad_producto').value) - 1">
                                        <svg width="12" height="2" viewBox="0 0 12 2" fill="none" xmlns="http://www.w3.org/2000/svg"><g opacity="0.35"><rect x="12" width="2" height="12" transform="rotate(90 12 0)" fill="currentColor"></rect></g></svg>
                                    </button>
                                    <input class="w-12 m-0 px-2 py-4 text-center md:text-right border-0 focus:ring-transparent focus:outline-none rounded-md" id="cantidad_producto" type="number" value="1">
                                    <button class="py-2 hover:text-gray-700" onclick="document.getElementById('cantidad_producto').value = parseInt(document.getElementById('cantidad_producto').value) + 1">
                                        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg"><g opacity="0.35"><rect x="5" width="2" height="12" fill="currentColor"></rect><rect x="12" y="5" width="2" height="12" transform="rotate(90 12 5)" fill="currentColor"></rect></g></svg>
                                    </button>
                                </div>
                            </div>
                            <?php
                        }
                        ?>
                        <!--<div>
                            <span class="block mb-4 font-bold font-heading text-gray-400 uppercase">Size</span>
                            <select class="pl-6 pr-10 py-4 font-semibold font-heading text-gray-500 border border-gray-200 focus:ring-blue-300 focus:border-blue-300 rounded-md" name="" id="">
                                <option value="1">Medium</option>
                                <option value="2">Small</option>
                                <option value="3">Large</option>
                            </select>
                        </div>-->
                    </div>
                    <?php
                    if ($producto_stock[$keyProducto] > 0) {
                        ?>
                        <div class="flex flex-wrap -mx-4 mb-14 items-center">
                            <div class="w-full xl:w-2/3 px-4 mb-4 xl:mb-0"><a class="block bg-orange-300 hover:bg-orange-400 text-center text-white font-bold font-heading py-5 px-8 rounded-md uppercase transition duration-200" href="#" onclick="anadirAlCarrito('<?php echo $producto; ?>', <?php echo $id_producto_sys; ?>, document.getElementById('cantidad_producto').value, '');">Añadir al carrito</a></div>
                            <!--<div class="w-full xl:w-1/3 px-4">
                                <a class="ml-auto sm:ml-0 flex-shrink-0 inline-flex mr-4 items-center justify-center w-16 h-16 rounded-md border hover:border-gray-500" href="#">
                                    <svg class="w-6 h-6" width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M13.4993 26.2061L4.70067 16.9253C3.9281 16.1443 3.41815 15.1374 3.24307 14.0471C3.06798 12.9568 3.23664 11.8385 3.72514 10.8505V10.8505C4.09415 10.1046 4.63318 9.45803 5.29779 8.96406C5.96241 8.47008 6.73359 8.14284 7.54782 8.00931C8.36204 7.87578 9.19599 7.93978 9.98095 8.19603C10.7659 8.45228 11.4794 8.89345 12.0627 9.48319L13.4993 10.9358L14.9359 9.48319C15.5192 8.89345 16.2327 8.45228 17.0177 8.19603C17.8026 7.93978 18.6366 7.87578 19.4508 8.00931C20.265 8.14284 21.0362 8.47008 21.7008 8.96406C22.3654 9.45803 22.9045 10.1046 23.2735 10.8505V10.8505C23.762 11.8385 23.9306 12.9568 23.7556 14.0471C23.5805 15.1374 23.0705 16.1443 22.298 16.9253L13.4993 26.2061Z" stroke="black" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
                                    </svg>
                                </a>
                                <a class="flex-shrink-0 inline-flex items-center justify-center w-16 h-16 rounded-md border hover:border-gray-500" href="#">
                                    <svg class="w-6 h-6" width="24" height="23" viewBox="0 0 24 23" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M2.01328 18.9877C2.05682 16.7902 2.71436 12.9275 6.3326 9.87096L6.33277 9.87116L6.33979 9.86454L6.3398 9.86452C6.34682 9.85809 8.64847 7.74859 13.4997 7.74859C13.6702 7.74859 13.8443 7.75111 14.0206 7.757L14.0213 7.75702L14.453 7.76978L14.6331 7.77511V7.59486V3.49068L21.5728 10.5736L14.6331 17.6562V13.6558V13.5186L14.4998 13.4859L14.1812 13.4077C14.1807 13.4075 14.1801 13.4074 14.1792 13.4072M2.01328 18.9877L14.1792 13.4072M2.01328 18.9877C7.16281 11.8391 14.012 13.3662 14.1792 13.4072M2.01328 18.9877L14.1792 13.4072M23.125 10.6961L23.245 10.5736L23.125 10.4512L13.7449 0.877527L13.4449 0.571334V1V6.5473C8.22585 6.54663 5.70981 8.81683 5.54923 8.96832C-0.317573 13.927 0.931279 20.8573 0.946581 20.938L0.946636 20.9383L1.15618 22.0329L1.24364 22.4898L1.47901 22.0885L2.041 21.1305L2.04103 21.1305C4.18034 17.4815 6.71668 15.7763 8.8873 15.0074C10.9246 14.2858 12.6517 14.385 13.4449 14.4935V20.1473V20.576L13.7449 20.2698L23.125 10.6961Z" fill="black" stroke="black" stroke-width="0.35"></path>
                                    </svg>
                                </a>
                            </div>-->
                        </div>
                        <?php
                    } else {
                       ?>
                        <div class="flex flex-wrap items-center justify-between">
                            <form id="notificarmeStock<?php echo $id_producto_sys; ?>">
                                <input type="hidden"
                                       value="<?php echo $tienda; ?>"
                                       name="notificacion[tienda]"/>
                                <input type="hidden"
                                       value="<?php echo $id_producto_sys; ?>"
                                       name="notificacion[id_producto]"/>
                                <?php if ($id_librador) { ?>
                                    <input type="hidden"
                                           value="<?php echo $id_librador; ?>"
                                           name="notificacion[id_librador]"/>
                                <?php } else { ?>
                                    <input type="email" class="w-auto my-3 px-4 py-2 border rounded-md"
                                           name="notificacion[email]"
                                           placeholder="Email"/>
                                <?php } ?>
                                <button class="inline-block w-full md:w-auto mb-4 md:mb-0 md:mr-4 text-center bg-orange-300 hover:bg-orange-400 text-white font-bold font-heading py-4 px-8 rounded-md uppercase"
                                        type="button"
                                        onclick="notificarExistaStock('notificarmeStock<?php echo $id_producto_sys; ?>')">
                                    <svg id="spinnerIcon<?php echo $id_producto_sys; ?>" style="display: none;" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                    Notificarme si hay stock
                                </button>
                            </form>
                        </div>
                    <?php
                    }
                    ?>
                    <!--<div class="flex items-center">
                        <span class="mr-8 text-gray-500 font-bold font-heading uppercase">SHARE IT</span>
                        <a class="mr-1 w-8 h-8" href="#">
                            <img src="yofte-assets/buttons/facebook-circle.svg" alt="">
                        </a>
                        <a class="mr-1 w-8 h-8" href="#">
                            <img src="yofte-assets/buttons/instagram-circle.svg" alt="">
                        </a>
                        <a class="w-8 h-8" href="#">
                            <img src="yofte-assets/buttons/twitter-circle.svg" alt="">
                        </a>
                    </div>-->
                </div>
            </div>
        </div>
        <!--<div>
            <ul class="flex flex-wrap mb-16 border-b-2">
                <li class="w-1/2 md:w-auto"><a class="inline-block py-6 px-10 bg-white text-gray-500 font-bold font-heading shadow-2xl" href="#">Description</a></li>
                <li class="w-1/2 md:w-auto"><a class="inline-block py-6 px-10 text-gray-500 font-bold font-heading" href="#">Customer reviews</a></li>
                <li class="w-1/2 md:w-auto"><a class="inline-block py-6 px-10 text-gray-500 font-bold font-heading" href="#">Shipping &amp; returns</a></li>
                <li class="w-1/2 md:w-auto"><a class="inline-block py-6 px-10 text-gray-500 font-bold font-heading" href="#">Brand</a></li>
            </ul>
            <h3 class="mb-8 text-lg font-bold font-heading text-blue-300">Summer collection and laoreet get</h3>
            <p class="max-w-2xl text-gray-500">I had interdum at ante porta, eleifend feugiat nunc. In semper euismod mi a accums lorem sad. Morbi at auctor nibh. Aliquam tincidunt placerat mollis. Lorem euismod dignissim, felis tortor ollis eros, non ultricies turpis.</p>
        </div>-->
    </div>
</section>