...
function pnAvailable() {
    ...
    if (window.isSecureContext) {
        ...
        alert('site have to run in secure context!');
    }
    return bAvailable;
}
...
async function pnRegisterSW() {
    navigator.serviceWorker.register('PNServiceWorker.js')
        .then((swReg) => {
            // registration worked
            alert('Registration succeeded. Scope is ' + swReg.scope);
        }).catch((error) => {
            // registration failed
    ...
    navigator.serviceWorker.getRegistration()
        .then(function(reg) {
            reg.unregister()
                .then(function(bOK) {
                    if (bOK) {
                        alert('unregister service worker succeeded.');
                    } else {
                        alert('unregister service worker failed.');
                    }
                });
        });
}
    ...
    navigator.serviceWorker.getRegistration()
        .then(function(reg) {
            reg.update()
                .then(function(bOK) {
                    if (bOK) {
                        alert('update of service worker succeeded.');
                    } else {
                        alert('update of service worker failed.');
                    }
                });
        });
}
