<input type="hidden" name="apartado" id="apartado" value="referencias" />
<div>
    <strong>Datos referencias base</strong>
    <?php
    $contador_elementos = 0;
    $id_producto_productos_referencias = $id_url;
    $id_productos_detalles_enlazado_productos_referencias = 0;
    $id_productos_detalles_multiples_productos_referencias = 0;
    $id_packs_productos_referencias = 0;
    require($_SERVER['DOCUMENT_ROOT']."/admin/productos/componentes/form-datos-referencias.php");
    ?>
</div>
<?php
$select_sys = "packs";
        ...
        foreach ($matriz_vertical_datos as $Key_vertical_datos => $valor_vertical_datos) {
            ...
            foreach ($matriz_horizontal_datos as $Key_horizontal_datos => $valor_horizontal_datos) {
                ...
                if($matriz_activo[$Key_vertical_datos][$Key_horizontal_datos] == 1) {
                    $referencias_base_mostrado = false;
                    foreach ($matriz_id_productos_packs as $key_id_productos_packs => $valor_id_productos_packs) {
                        ?>
                        <input type="hidden" name="id_productos_referencias[]" value="<?php echo $matriz_id[$Key_vertical_datos][$Key_horizontal_datos]; ?>" />
                        <?php
                        if (!empty($matriz_id_productos_detalles_enlazado_productos_packs[$key_id_productos_packs])) {
                            if ($matriz_id[$Key_vertical_datos][$Key_horizontal_datos] == $matriz_id_productos_detalles_enlazado_productos_packs[$key_id_productos_packs]) {
                                $descripcion_pack_datos_referencias = "<strong>PACK: ".$matriz_cantidad_pack_productos_packs[$key_id_productos_packs]."&nbsp;units.</strong><br />";
                                ...
                                $id_producto_productos_referencias = $id_url;
                                $id_productos_detalles_enlazado_productos_referencias = $id_productos_detalles_enlazado_url;
                                $id_productos_detalles_multiples_productos_referencias = 0;
                                $id_packs_productos_referencias = $id_packs_url;
                                require($_SERVER['DOCUMENT_ROOT']."/admin/productos/componentes/form-datos-referencias.php");
                                unset($descripcion_pack_datos_referencias);
                                unset($id_packs_url);
                            }else if($referencias_base_mostrado == false) {
                                $referencias_base_mostrado = true;
                                $id_productos_detalles_enlazado_url = $matriz_id[$Key_vertical_datos][$Key_horizontal_datos];
                                $id_producto_productos_referencias = $id_url;
                                $id_productos_detalles_enlazado_productos_referencias = $id_productos_detalles_enlazado_url;
                                $id_productos_detalles_multiples_productos_referencias = 0;
                    ...
                    if($referencias_base_mostrado == false){
                        $id_productos_detalles_enlazado_url = $matriz_id[$Key_vertical_datos][$Key_horizontal_datos];
                        $id_producto_productos_referencias = $id_url;
                        $id_productos_detalles_enlazado_productos_referencias = $id_productos_detalles_enlazado_url;
                        $id_productos_detalles_multiples_productos_referencias = 0;
    ...
    foreach ($matriz_id_productos_packs as $key_id_productos_packs => $valor_id_productos_packs) {
        ...
        if(!empty($descripcion_pack)) {
            ...
            if($matriz_activo_productos_packs[$key_id_productos_packs] == 1) {
                $descripcion_pack_datos_referencias = "<strong>Pack de: " . $descripcion_pack."&nbsp;".$matriz_cantidad_pack_productos_packs[$key_id_productos_packs]."&nbsp;unid.</strong>";
            }else {
                $descripcion_pack_datos_referencias = "Pack de: " . $descripcion_pack."&nbsp;unid.";
            }
            $id_productos_detalles_multiples_url = $id_productos_detalles_multiples;
            $id_packs_url = $valor_id_productos_packs;
            $id_producto_productos_referencias = $id_url;
            $id_productos_detalles_enlazado_productos_referencias = 0;
            $id_productos_detalles_multiples_productos_referencias = $id_productos_detalles_multiples;
            $id_packs_productos_referencias = $id_packs_url;
            require($_SERVER['DOCUMENT_ROOT']."/admin/productos/componentes/form-datos-referencias.php");
            unset($descripcion_pack_datos_referencias);
            unset($id_packs_url);
            ...
            if($matriz_activo_productos_packs[$key_id_productos_packs] == 1) {
                $descripcion_pack_datos_referencias = "<strong>Pack de: " . $matriz_cantidad_pack_productos_packs[$key_id_productos_packs]."&nbsp;unid.</strong>";
            }else {
                $descripcion_pack_datos_referencias = "Pack de: " . $matriz_cantidad_pack_productos_packs[$key_id_productos_packs]."&nbsp;unid.";
            }
            $id_packs_url = $valor_id_productos_packs;
            $id_producto_productos_referencias = $id_url;
            $id_productos_detalles_enlazado_productos_referencias = 0;
...
if(isset($productos_detalles_multiples_id_atributo)) {
    foreach ($matriz_productos_detalles_multiples_id as $key_productos_detalles_multiples_id => $valor_productos_detalles_multiples_id) {
        if($matriz_productos_detalles_multiples_activo[$key_productos_detalles_multiples_id] == 1) {
            ...
            $select_sys = "descripcion_multiple";
            require($_SERVER['DOCUMENT_ROOT']."/admin/productos/detalles/gestion/datos-select-php.php");
            $descripcion_pack_datos_referencias = $descripcion_productos_detalles_multiples;
            $id_producto_productos_referencias = $id_url;
            $id_productos_detalles_enlazado_productos_referencias = 0;
            $id_productos_detalles_multiples_url = $valor_productos_detalles_multiples_id;
            $id_productos_detalles_multiples_productos_referencias = $valor_productos_detalles_multiples_id;
            $id_packs_productos_referencias = 0;
            require($_SERVER['DOCUMENT_ROOT']."/admin/productos/componentes/form-datos-referencias.php");
            unset($descripcion_pack_datos_referencias);
        }
