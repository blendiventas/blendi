...
if(isset($matriz_id_categorias_elaborados)) {
    ...
    <div id="capa_listado_resultados" class="overflow-y-auto bg-white sm:mx-5">
        <?php
        foreach ($matriz_id_categorias_elaborados as $key => $valor) {
            ?>
            <div class="grid grid-cols-12 items-center h-14 bg-white border-2 border-gray-50" id="linea_<?php echo $valor; ?>">
                <div class="px-3 col-span-10 sm:col-span-10"><?php echo $matriz_descripcion_categorias_elaborados[$key]; ?></div>
                <div class="col-span-2 px-3">
                    <a href="#" class="botones-apartados" title="<?php echo $matriz_descripcion_categorias_elaborados[$key]; ?>" onclick="abrirFicha(<?php echo $valor; ?>)">
    ...
    <div class="flex items-center justify-center h-10 bg-white mx-5">
        <div class="text-center grow px-3">
            No existen categorías de elaborado definidas.
        </div>
    </div>
    <?php
}
