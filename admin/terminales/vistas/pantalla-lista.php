...
if(isset($matriz_id_terminales)) {
    ...
    <div id="capa_listado_resultados" class="overflow-y-auto bg-white sm:mx-5">
        <?php
        foreach ($matriz_id_terminales as $key => $valor) {
            ?>
            <div class="grid grid-cols-12 items-center h-14 bg-white border-2 border-gray-50" id="linea_<?php echo $valor; ?>">
                <div class="hidden sm:block sm:col-span-2 px-3" id="capa_img_activo_<?php echo $valor; ?>">
                    <input type="checkbox" class="block w-7 h-7 mx-auto text-blendi-600" id="habilitar_<?php echo $valor; ?>"
                        <?php echo ($matriz_activo_terminales[$key] == 1)? ' checked ' : ''; ?>
                           onmouseover="this.style.cursor='pointer'" onclick="toogleHabilitar('<?php echo $valor; ?>', 'terminales');" />
                </div>
                <div class="px-3 col-span-3 sm:col-span-8"><?php echo $matriz_descripcion_terminales[$key]; ?></div>
                <div class="col-span-2 px-3">
                    <a href="#" class="botones-apartados" title="<?php echo $matriz_descripcion_terminales[$key]; ?>" onclick="abrirFicha(<?php echo $valor; ?>)">
        ...
        unset($matriz_activo_terminales);
        ?>
    </div>
    <?php
    ...
    <div class="flex items-center justify-center h-10 bg-white mx-5">
        <div class="text-center grow px-3">
            No existen terminales definidos.
        </div>
    </div>
    <?php
}
